...
export default async function serialize(client, msg) {
	...
	if (msg.key) {
		...
		const remoteJid = (isLid ? (m.key.remoteJidAlt || msg.remoteJidAlt || m.key.remoteJid) : m.key.remoteJid) || m.key.remoteJid;
	...
	const finalSender = ((m.sender || '').split('@')[0] || '').replace(/[^0-9]/g, '');

	const owner = ((client.user?.id?.split(':')[0] || '')).replace(/[^0-9]/g, '');
	...
	const isOwnerCheck = (finalSender && finalSender === owner) || ownerNumbers.includes(finalSender);
	m.isOwner = isOwnerCheck

	if (m.isGroup) {
		m.metadata = normalizeGroupMeta(await client.groupMetadata(m.from));
	...
	if (m.message) {
		...
		m.body = getTextFromMessage(m.msg);
		m.prefix = config.bot.prefix
		m.isCmd = m.body.startsWith(config.bot.prefix)

		m.command = m.isCmd
			? m.body.slice(m.prefix.length).trim().split(/ +/).shift().toLowerCase()
			: null
		m.args = m.isCmd
