...
export const upload = {
	...
	async bashUpload(media) {
		try {
			...
			const uploadUrl = `https://bashupload.app/${fileName}`;
			const { data } = await axios.put(uploadUrl, media, {
				headers: {
					'Content-Type': mime,
					'X-Expiration-Seconds': '180'
				}
			});
			const urlMatch = data.match(/(https?:\/\/[^\s]+)/);

			// Jika ketemu, ambil index ke-0 (URL-nya), jika tidak kembalikan data mentahnya
			const finalUrl = urlMatch ? urlMatch[0] : data;

			return finalUrl;
		} catch (error) {
			console.error('Error uploading file in baseuplo:', error);
		}
	},
	async tmpFiles(media) {
		try {
			media = Buffer.isBuffer(media) ? media : readFileSync(media);
			const { ext } = await fileTypeFromBuffer(media);
			const form = new FormData();
			...
			console.error('Error uploading file in tmpfile:', error);
		}
	},
...
export async function sendTelegram(chatId, data, options = {}) {
    try {
        // 1. Aman dari config kosong
        let token = config?.tele?.TELEGRAM_TOKEN;
        if (!token) {
            console.log("⚠️ Token Telegram tidak ditemukan di config.");
            return null;
        }
        ...
        if (!type) {
            if (typeof data === 'string') {
                ...
                else if (/image/.test(mime)) type = 'photo';
                else if (/video/.test(mime)) type = 'video';
                else if (/opus/.test(mime)) type = 'voice';
                else if (/audio/.test(mime)) type = 'audio';
                else type = 'document';
            } else {
                // Jika bukan string dan bukan buffer, jadikan teks agar tidak crash
                type = 'text';
                data = String(data);
            }
        } else if (Buffer.isBuffer(data)) {
            // Jika 'type' dipaksa dari options, tetap cari ekstensinya
            let fileType = await fileTypeFromBuffer(data);
            fileExt = fileType?.ext || 'bin';
        }
        ...
        if (type === 'text') {
            // 3. Telegram API menggunakan parameter 'text' untuk sendMessage
            form.append('text', data); 
        } else {
            let filename = `file-${Date.now()}.${DEFAULT_EXTENSIONS[type] || fileExt}`;
        ...
        let { data: response } = await axios.post(url, form, {
            headers: {
                ...(typeof form.getHeaders === 'function' ? form.getHeaders() : { 'Content-Type': 'multipart/form-data' })
            },
        });

        return response;
    } catch (e) {
        console.error("❌ Error sendTelegram:", e.response?.data || e.message);
        return null;
    }
}
...
export function cutStr(str, len = 40) {
